...
def solve_gauss_newton_operator(
    problem: Any,
    max_iters: int = 20,
    *,
    x0: Array | Any = None,
    ...
    damping_min_factor: float = 100.0,
    c_armijo: float = 1e-4,
    history: bool = True,
    ...
    Uses jvp/vjp and damped CGLS, never a stacked Jacobian in the solver.
    ...
    roundoff acceptance, callbacks and gradient-based convergence.
    """

    if not np.isfinite(inner_tol) or not 0 < inner_tol < 1:
        raise ValueError("inner_tol must be finite and in (0, 1).")
    if inner_max_iters is not None and (
        not np.isfinite(inner_max_iters) or int(inner_max_iters) != inner_max_iters
        or inner_max_iters <= 0
    ):
        raise ValueError("inner_max_iters must be a positive integer.")
    ...
    if line_search:
        if not np.isfinite(damping_decrease) or not 0 < damping_decrease < 1:
            raise ValueError("solve_gauss_newton_operator: damping_decrease must be finite and in (0, 1).")
        if not np.isfinite(c_armijo) or not 0 < c_armijo < 1:
            raise ValueError("solve_gauss_newton_operator: c_armijo must be finite and in (0, 1).")
        if not np.isfinite(ls_max_retries) or int(ls_max_retries) != ls_max_retries or ls_max_retries < 0:
            raise ValueError("solve_gauss_newton_operator: ls_max_retries must be a nonnegative integer.")
        if not np.isfinite(damping_increase) or damping_increase <= 1:
            raise ValueError("solve_gauss_newton_operator: damping_increase must be finite and > 1.")
        if not np.isfinite(damping_max) or damping_max <= 0:
            raise ValueError("solve_gauss_newton_operator: damping_max must be finite and > 0.")
        if not np.isfinite(ls_beta) or not 0 < ls_beta < 1:
            raise ValueError("solve_gauss_newton_operator: ls_beta must be finite and in (0, 1).")
        if not np.isfinite(ls_min_step) or ls_min_step <= 0:
            raise ValueError("solve_gauss_newton_operator: ls_min_step must be finite and > 0.")
    ...
    def _damping_floor(J: Any) -> float:
        ...
        if not np.isfinite(floor):
            raise ValueError(
                "solve_gauss_newton_operator: cannot determine a finite damping lower "
                "bound from J.T @ J."
            )
        return floor
        ...
        if recorder.active and k > last_recorded_iteration:
            metadata = {} if k else {
                ...
                "settings": {
                    ...
                    "inner_tol": inner_tol, "inner_max_iters": inner_max_iters,
            ...
            recorder.emit(
                ...
                **(search_summary if k else {}),
                ...
                damping_min=current_damping_floor,
            )
    ...
    def _outcome(
        *,
        ...
        final_linearization: tuple[Array, Any] | None = None,
    ) -> SolveOutcome:
        nonlocal current_damping, current_damping_floor
    ...
    for k in range(int(max_iters)):
        ...
        while True:
            ...
            if search_reason == "tiny_update" and current_damping > 0 and not reduced_damping:
                current_damping = max(
                    current_damping_floor, current_damping * damping_decrease,
                )
                ...
                # A scale-aware lower bound takes priority over an absolute cap:
                # an absolute cap below roundoff would reintroduce an effectively
                # undamped step for a large-scale Jacobian.
                damping_cap = max(float(damping_max), current_damping_floor)
                if current_damping >= damping_cap:
                    stop_reason = "damping_limit"
                    break
                next_damping = min(
                    damping_cap,
                    max(current_damping_floor, current_damping * damping_increase),
                )
