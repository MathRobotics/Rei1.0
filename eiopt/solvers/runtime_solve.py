...
def _set_runtime_x(runtime: ProblemRuntime, x: Array) -> None:
    pack = runtime.pack
...
def solve_scipy_minimize(
    ...
    *,
    ...
    """Solve `||r(x)||^2` from a `ProblemRuntime` via scipy.optimize.minimize."""
...
def solve_cyipopt_minimize(
    ...
    *,
    ...
    """Solve `||r(x)||^2` from a `ProblemRuntime` via cyipopt.minimize_ipopt."""
    ...
    try:
        ...
        if "callback" not in str(e):
            raise
        # Older cyipopt variants may not support callback in this interface.
